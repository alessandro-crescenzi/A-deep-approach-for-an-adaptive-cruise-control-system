...
parser.add_argument('--keep_latest_interval', default=100000, type=int,
                    help='When --keep_latest is on, don\'t delete the latest file at these intervals. This should be '
                         'a multiple of save_interval or 0.')
...
parser.add_argument('--batch_alloc', default=None, type=str,
                    help='If using multiple GPUS, you can set this to be a comma separated list detailing which GPUs '
                         'should get what local batch size (It should add up to your total batch size).')
parser.add_argument('--no_autoscale', dest='autoscale', action='store_false',
                    help='YOLACT will automatically scale the lr and the number of iterations depending on the batch '
                         'size. Set this if you want to disable that.')
...
def train():
    ...
    yolact_net.load_weights("weights/yolact_resnet50_54_800000.pth")
    yolact_net.load_weights("weights/yolact_base_5_2226.pth")
    ...
    if args.validation_epoch > 0:
        compute_validation_loss(yolact_net, val_dataset, optimizer)
...
def compute_validation_loss(yolact_net, val_dataset, optimizer):
    global loss_types
    ...
    print(('Validation Loss ||' + (' %s: %.3f |' * len(losses)) + ' T: %.3f ' + ')') % tuple(loss_labels + [total]),
          flush=True)
