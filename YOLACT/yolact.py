            ...
            def make_extra(num_layers):
                if num_layers == 0:
                    ...
                    return nn.Sequential(*sum([[
                        nn.Conv2d(243, 33, kernel_size=3, padding=1),
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        # self.load_weights("./weights/yolact_resnet50_54_800000.pth")
    ...
    def load_weights(self, path):
        ...
        for key in list(state_dict.keys()):
            ...
            # if (key.startswith('prediction_') or key.startswith('semantic_')) and path.endswith("yolact_resnet50_54_800000.pth"):
            #    del state_dict[key]

            # if key.startswith('prediction_layers.0.conf_layer.weight') and path.endswith("yolact_resnet50_54_800000.pth"):
            #     traffic_sign_weights = state_dict[key][36:39, :, :, :].clone()
            #     state_dict[key] = state_dict[key][:33, :, :, :]
            #     state_dict[key][15:18, :, :, :] = state_dict[key][3:6, :, :, :]
            #     state_dict[key][27:30, :, :, :] = traffic_sign_weights
            #
            # if key.startswith('prediction_layers.0.conf_layer.bias') and path.endswith("yolact_resnet50_54_800000.pth"):
            #     traffic_sign_bias = state_dict[key][36:39].clone()
            #     state_dict[key] = state_dict[key][:33]
            #     state_dict[key][15:18] = state_dict[key][3:6]
            #     state_dict[key][27:30] = traffic_sign_bias
            #
            # if key.startswith('semantic_seg_conv.weight') and path.endswith("yolact_resnet50_54_800000.pth"):
            #     traffic_sign_weights = state_dict[key][11, :, :, :].clone()
            #     state_dict[key] = state_dict[key][:10, :, :, :]
            #     state_dict[key][4, :, :, :] = state_dict[key][0, :, :, :]
            #     state_dict[key][8, :, :, :] = traffic_sign_weights
            #
            # if key.startswith('semantic_seg_conv.bias') and path.endswith("yolact_resnet50_54_800000.pth"):
            #     traffic_sign_bias = state_dict[key][11].clone()
            #     state_dict[key] = state_dict[key][:10]
            #     state_dict[key][4] = state_dict[key][0]
            #     state_dict[key][8] = traffic_sign_bias
