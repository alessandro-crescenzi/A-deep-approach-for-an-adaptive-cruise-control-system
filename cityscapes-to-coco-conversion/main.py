...
def instances2dict_with_polygons(imageFileList, verbose=False):
    ...
    for imageFileName in imageFileList:
        ...
        for instanceId in np.unique(imgNp):
            if instanceId > 1000 or instanceId == 19 or instanceId == 20:
                ...
                if instanceId == 19 or instanceId == 20:
                    for i in range(len(polygons)):
                        tmp = instanceObj_dict.copy()
                        tmp['instID'] = (instanceObj_dict['instID'] * 1000) + i
                        tmp['contours'] = [polygons[i]]
                        instances[id2label[instanceObj.labelID].name].append(tmp)
