from utils.frr.base import dct2, idct2, laplacian
from utils.frr.utils import FileWriter, min_max_scale
import numpy as np
...
class FastReflectionRemoval:
    ...
    def __init__(self, M, N, h: float, lmbd: float = 0, mu: float = 1, epsilon: float = 1e-8, debug_writer: FileWriter = None):
        """
        Args:
        ...
        self.kappa = 2 * (np.add.outer(m, n) - 2)

        self.const = self.mu * (self.kappa ** 2) - self.lmbd * self.kappa + self.epsilon
    ...
    def _compute_T(self, rhs: np.ndarray) -> np.ndarray:
        ...
        for c in range(channels):
            ...
            if self.debug_writer:
                self.debug_writer.save_image(self.kappa, name=f"{c}", category="kappa")

            if self.debug_writer:
                self.debug_writer.save_image(self.const, name=f"{c}", category="denominator")
            ...
            if self.debug_writer:
                self.debug_writer.save_image(self.const, name=f"{c}", category=f"T")
